...
total_inactive,SUM(IF (en.status='1',1,0)) AS total_convert FROM ugc_entry ugc left join entry en ON ugc.entryid=en.entryid  ";
$tableAD=  db_select($conn,$act_inact);
$totalActive=$tableAD[0]['total_active']; $totalInactive=$tableAD[0]['total_inactive']; $total_convert=$tableAD[0]['total_convert'];
$totalEntry=$totalActive+$totalInactive+$total_convert;
...
<div class="box-header" >
  ...
  <td width="32%" align="center">
      ...
      <span class="label label-default">PENDING <span class="badge"  style="color: #444; background-color: #fff;padding: 0px 4px 1px 4px;"><?php echo $totalInactive; ?></span></span>
...
foreach($fetchMedia as $entry_media) {
    ...
    if($status==11) { $statusc="Pending";  $redyColor='label-warning';  $disableLink='';}
    else{ 
    if($kstatus=='-1') { $statusc="error_converting";  $redyColor='label-warning';  $disableLink='not-active-href';}
    if($kstatus=='-2') { $statusc="error_importing";  $redyColor='label-warning';  $disableLink='not-active-href';}
    if($kstatus==0) { $statusc="import";  $redyColor='label-warning';  $disableLink='not-active-href';}
    if($kstatus==1) { $statusc="converting";  $redyColor='label-warning';  $disableLink='not-active-href';}
    if($kstatus==2) { $statusc="Ready"; $redyColor='label-success';  $disableLink='not-active-href';}
    }
?>
