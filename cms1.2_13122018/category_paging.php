...
case "refresh":
    ...
    function categoryChild($categoryid,$cstatus) {
    global $conn;   
    $sqry = "SELECT category_id FROM categories WHERE parent_id = '$categoryid' and status!='3'";
    echo "total=".$totalRowC=  db_totalRow($conn,$sqry);
    $children = array();
        if($totalRowC > 0) {
            $fetchCat=  db_select($conn,$sqry);
            foreach($fetchCat as $row){
             $catidu=$row['category_id'];
             $upstatus=$cstatus=='2'?'1':'2';
             echo $upchild="update categories set status='$upstatus' where category_id='$catidu'"; echo "<br/>";
             db_query($conn,$upchild);
             $children[$row['category_id']] = categoryChild($row['category_id'],$cstatus);
            }
        }
    //return $children;
    }    
    echo $categoryid= (isset($_POST['category_id']))? $_POST['category_id']: "";
    $cparentid= (isset($_POST['parentid']))? $_POST['parentid']: "";
    $catStatus= (isset($_POST['catStatus']))? $_POST['catStatus']: "";
    $getAllCategory=categoryChild($categoryid,$catStatus); print_r($getAllCategory);
    $upstatus=$catStatus=='2'?'1':'2';
    echo $upcmain="update categories set status='$upstatus' where category_id='$categoryid'";echo "<br/>";
    db_query($conn,$upcmain);
    break;    
...
<div class="box-header" >
    ...
    <tr>
    <td width="12%"><select id="pagelmt"  style="width:60px;" onchange="selpagelimit('<?php echo $page;  ?>','<?php echo $searchTextMatch;?>');" >
    ...
    <td width="2%">
        <input type="button" class="btn-primary btn-xs" title="active/inactive category" value="Active/Inactive" onclick="activeInactive();" ></td>
    <td width="35%">
    ...
    <td width="2%">
     <div class="  pull-right" style="border:0px solid red;  margin-top:1px !important;">   
      <a href="javascript:" onclick="return refreshcontent('refresh','<?php echo $page;  ?>','<?php echo $pagelimit; ?>','<?php echo $searchTextMatch;?>');" title="Sync" style="float: right"><i class="fa fa-refresh" aria-hidden="true"></i> Refresh</a>   
    </div>
     </td>
    </tr>
</table>	  
...
<div class="dropdown">
 ...
 <a href="javascript:void(0)" class="myBtnn" onclick="SetEntryPriorty('<?php echo $id; ?>','<?php echo $name; ?>')">Pri
  <!--<i class="fa fa-edit" aria-hidden="true" data-toggle="tooltip" data-placement="left"  title="View Entries"></i>-->
 </a>
...
function categoryActiveInactive(catid,parentid,catStatus,pageindex,limitval,searchtext)
{
     $('#load').show();
     $('#results').css("opacity",0.1);
     var apiBody = new FormData();
     apiBody.append("category_id",catid);
     apiBody.append("parentid",parentid);
     apiBody.append("catStatus",catStatus);
     apiBody.append("pageNum",pageindex);
     apiBody.append("limitval",limitval);
     apiBody.append("searchInputall",searchtext);
     apiBody.append("categoryaction","categoryactiveiactive");
     $.ajax({
           type: "POST",
           url: "category_paging.php",
           data: apiBody,
