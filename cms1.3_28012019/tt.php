...
if($entryFound==0)
{
  ...
  // update ispremium in entry table
   ...
   foreach ($ckdentryid  as $eid) {
     echo $qryEntry="update entry set ispremium='1',planid='c' where FIND_IN_SET('$categoryid',categoryid) and status='2' and entryid='$eid'";
     // db_query($conn,$qryEntry);
     }
     $unckdentryid=explode(","$uncheckedEntryId);
     foreach ($unckdentryid  as $uneid) {
     echo  $qryEntry="update entry set ispremium='0',planid='c' where FIND_IN_SET('$categoryid',categoryid) and status='2' and entryid='$uneid'";
       // db_query($conn,$qryEntry);
       }
