...
$("#preview_i_edit").html('Uploading.... <img src="img/image_process.gif" alt="Uploading...."/>');
     ...
     $.ajax({
        ...
        function checkInput(){
        var x= document.getElementById("entryname").value.replace(/[^a-z0-9\s]/gi, '').replace(/\s+/g, '').toLowerCase();
        document.getElementById("cat_alias").value=x;
        }
</script>
...
<div class="tab-content">
       ...
       <div style=" border:1px solid #c7d1dd;  border-top: 0 none !important;margin-left: 15px; padding: 28px; width: 870px;">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-5">
                 <input type="text" class="form-control" id="entryname" placeholder="Entry Name" onkeyup="checkInput()" value="<?php echo  ($name); ?>">
            </div>
        </div>
        <div class="form-group">
             <label for="inputEmail" class="control-label col-xs-4">Short Name :</label>
             <div class="col-xs-5">
                  <input type="text" class="form-control" id="cat_alias" name="cat_alias"  disabled placeholder="Category Alias/ Short Name" size="100" value="<?php echo strtolower(preg_replace('/[^A-Za-z0-9\-]/', '', $name)); ?>">
             </div>
       </div>
         <!--<div class="form-group">
           <label for="inputtagname" class="control-label col-xs-4">Category Type:</label>
           <div class="col-xs-5">
         <select id="tag_name"  style="" onchange="" >
             <option value="N" >Normal Category</option>
             <option value="WS">Web Series Category</option>
             <option value="S">Seasons Category</option>
             </select>
       </div></div>-->
         <div class="form-group">
...
function save_category_in_server(caction){
   var catid = $('#catid').val();
    ...
    var cat_alias=$("#cat_alias").val();

    var save_edit_category="save_edit_category";
    var dataString ='categoryaction='+save_edit_category+'&catid='+catid+'&entryname='+entryname+'&entrydesc='+entrydesc+'&entrytags='+entrytags+'&pageindex='+pageindex+'&limitval='+limitval+'&cat_alias='+cat_alias;
