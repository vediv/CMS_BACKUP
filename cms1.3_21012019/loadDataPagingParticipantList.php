...
$act_inact="SELECT  SUM(IF (status='1',1,0)) AS total_active,SUM(IF (status='0',1,0)) AS
total_inactive FROM user_registration ";
...
<div class="box-header" >
    ...
    <select id="pagelmt"  style="width:60px;" onchange="selpagelimit('pagelmt','loadDataPagingParticipantList.php','load');" >
       <option value="3"  <?php echo $pagelimit==3? "selected":""; ?> >3</option>
...
if($searchKeword!='')
{
    $query_search = " and (dt.RegCode LIKE '%" . $searchKeword . "%' or dt.name LIKE '%" . $searchKeword . "%'or ur.ucountry LIKE '%" . $searchKeword . "%'or ur.mobile LIKE '%" . $searchKeword . "%')";
}
...
if($page)
        ...
        $start = 0;
$sql="SELECT dt.userid,dt.name,dt.RegCode,dt.regDate,ur.mobile,ur.ucountry,ur.dob FROM
device_token dt LEFT JOIN user_registration ur ON dt.userid = ur.uid
WHERE dt.status='1' $query_search";
...
foreach($que as $fetch)
{
  ...
  $mobile=$fetch['mobile']; $reg_date=$fetch['regDate'];$country=$fetch['ucountry'];$dob=$fetch['dob'];
