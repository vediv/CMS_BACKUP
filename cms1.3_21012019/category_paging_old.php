...
$shortName	 = (isset($_POST['cat_alias']))? $_POST['cat_alias']: null;
...
$insert="insert into categories(category_id,parent_id,dept,partner_id,cat_name,fullname,fullids,entry_count,direct_sub_categories_count,description,tags,created_at,updated_at,duser_id,priority,direct_entries_count,short_name)
Select '$CatID','$parentId','$depth','$partnerID','$catname','$cfullName','$fullIds','$entriesCount','$directSubCategoriesCount',$cdescription,$ctags,NOW(),Now(),'$get_user_id',ifnull(max(priority),0)+1,'$directEntriesCount','$shortName' from categories";
...
case "save_edit_category":
         ...
         $shortName = (isset($_POST['cat_alias']))? $_POST['cat_alias']: '';
         $id = $category_ID;
         $category = new KalturaCategory();
         $category->name = $cat_name;
         $category->description = $cat_description;
         ...
         if($entryFound==1)
         {
             echo $upc="update categories set cat_name='$c_name',description=$c_description,tags=$c_tags,fullname='$c_fullName',short_name='$shortName' where category_id='$category_ID'";
         ...
         if($entryFound>1)
         {
              ...
              if($parent_id==0)
              {
                 ...
                 $upc1="update categories set cat_name='$c_name',description=$c_description,tags=$c_tags,fullname='$c_fullName',short_name='$shortName' where category_id='$category_ID'";
                 db_query($conn,$upc1);
              }
              if($parent_id>0)
              {
                 ...
                 $upc2="update categories set cat_name='$c_name',description=$c_description,tags=$c_tags,fullname='$c_fullName',short_name='$shortName' where category_id='$category_ID'";
                 db_query($conn,$upc2);
...
case "categoryactiveiactive":
 ...
 if($entryFound==0)
 {
   $qry="insert into category_plan_info(categoryid,planid,created_at)values('$categoryid','$planid',NOW())";
   $exe=db_query($conn,$qry);
   // update ispremium in entry table
   $qryEntry="update entry set ispremium='1' where FIND_IN_SET('$categoryid',categoryid) and status='2'";
...
if($entryFound==1){
       $cpi=$getData[0]['cpi'];
       $qry="update category_plan_info set categoryid='$categoryid',planid='$planid',updated_at=NOW() where cpi='$cpi' ";
       $exe=db_query($conn,$qry);
   /*----------------------------update log file begin-------------------------------------------*/
...
/*$sql="SELECT cat.catid,cat.category_id,cat.cat_name,cat.parent_id,cat.created_at,cat.priority,cat.fullname,
    ...
    cti.host_url_thumb,cti.host_url_icon,cpi.planid FROM categories  cat
    LEFT JOIN category_thumb_icon_url cti ON cat.category_id = cti.category_id
    LEFT JOIN category_plan_info cpi ON cat.category_id = cpi.categoryid
    where cat.status!='3'  ";*/
    $sql="SELECT cat.catid,cat.category_id,cat.cat_name,cpi.tag,cat.parent_id,cat.created_at,cat.priority,cat.fullname,
        cat.direct_sub_categories_count,cat.direct_entries_count,cat.status,cat.priority,cti.t_mediam_url,cti.t_small_url,cti.i_small_url,
...
<td></td>
<?php  if(in_array(21, $otherPermission)){ ?><td width="10%"><?php echo $imgthumb; ?></td> <?php } ?>
...
<?php echo wordwrap($name,40, "\n", true); ?>
  ...
  <?php  if(in_array(40, $otherPermission)){ ?><td><?php echo  $plan_tag; ?></td><?php } ?>
...
<td>
  ...
  <ul class="dropdown-menu" style="border:0px red solid; margin: 0 0 0 -10px !important">
     <li>
       <?php   if(in_array("3", $UserRight)){ ?>
       <a href="javascript:void(0)" class="myBtnn" onclick="view_category_entry('<?php echo $id; ?>')"><i class="fa fa-eye" aria-hidden="true" data-toggle="tooltip" data-placement="left"  title="View Entries"></i>View Entries</a>
     ...
     <li>
      ...
      <?php } if(in_array("2", $UserRight)){ ?>
       <a href="javascript:void(0)" class="myBtn" onclick="categoryEdit('<?php echo $id; ?>','<?php echo $page; ?>','<?php echo $pagelimit; ?>')"><i class="fa fa-pencil-square-o" aria-hidden="true" data-toggle="tooltip" data-placement="left"  title="Edit" style="padding-left: 8px  !important;"></i>Edit</a>
     </li>
      <li>
        <a href="javascript:void(0)" class="myBtnn" onclick="SetEntryPriorty('<?php echo $id; ?>','<?php echo $name; ?>')">
        <i class="fa fa-level-up" aria-hidden="true" title="Set Video Priority" style="padding-left: 8px  !important;"></i>
        Set Priority</a>
     </li>
      <li>
        <?php
        ...
        <i class="fa fa-usd" aria-hidden="true" title="add Plan" style="padding-left: 8px  !important;"></i>
        Add Plan</a>
     </li>
