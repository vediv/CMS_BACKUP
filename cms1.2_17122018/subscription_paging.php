...
<div class="col-sm-3 col-md-3 pull-right navbar-form" role="search">  
            ...
            <input class="form-control" size="30" onkeyup="SeachDataTable('subscription_paging.php','<?php echo $pagelimit;?>','<?php //echo $page; ?>','load')"  placeholder="Search By UserID,OrderID,Trans"  autocomplete="off" name='searchQuery' id='searchInput' class="searchInput" type="text" value="<?php echo $searchKeword; ?>">
...
<td width="3%">
     <div class="col-xs-1 hidden-xs hidden-sm pull-right" style="border:none;  margin-top:5px !important;">   
...
<td width="15%" align="right"><a href="report.php?code=subscription_detail" title="Detail Report">Detail Report</a></td>
</tr>
</table>
<div class="">
...
$sql = "SELECT upd.*,MAX(upd.added_date) added_date_new,ur.uid,ur.user_id
             FROM user_payment_details upd left join user_registration ur ON upd.userid=ur.uid 
             where upd.order_status!='Aborted' $query_search  GROUP BY upd.userid ORDER BY upd.added_date DESC LIMIT $start, $limit";
...
<thead>
    <tr>
           ...
           <th>Expire Date</th>
           <th>Order Status</th>
           <th title="Subscription Type">Subs Type</th>
...
foreach($que as $fetch)
{
    ...
    $trans_date=$fetch['trans_date']; $added_date_new=$fetch['added_date_new'];$paymentFor=$fetch['payment_for'];
    $uname=$fetch['user_id'];
    if($paymentFor=='plan'){ $subsType="Plan Subscription"; }
    if($paymentFor=='wallet'){ $subsType="Pay per View"; }
    if($paymentFor=='subs_code'){ $subsType="Subscription Code"; }
...
<tr class="detail-row">
    <td colspan="14">
        ...
        <?php
        $c=1;
        $sql="SELECT upd.*,ur.uid,ur.user_id,ur.uemail
        FROM user_payment_details upd left join user_registration ur ON upd.userid=ur.uid 
        where upd.userid='".$userid."' and orderid!='$orderid'  order by added_date DESC"  ;
        $que = db_select($conn,$sql);
        echo "Total=" .$countRow=  db_totalRow($conn,$sql);
        ?>
...
foreach($que as $fetch)
{
    $userid=$fetch['userid'];  $orderid1=$fetch['orderid']; $trans_id=$fetch['trans_id']; $order_status=$fetch['order_status']; 
    $payment_mode=$fetch['payment_mode']; $status_code=$fetch['status_code']; $currency=$fetch['currency']; 
    $amount=$fetch['amount']; $plan_days=$fetch['plan_days']; $added_date1=$fetch['added_date'];$expire_date=$fetch['expire_date'];
    $trans_date=$fetch['trans_date']; $added_date_new=$fetch['added_date_new'];
    $uname=$fetch['user_id']; $paymentFor1=$fetch['payment_for'];
    if($paymentFor1=='plan'){ $subsType1="Plan Subscription"; }
    if($paymentFor1=='wallet'){ $subsType1="Pay per View"; }
    if($paymentFor1=='subs_code'){ $subsType1="Subscription Code"; }
