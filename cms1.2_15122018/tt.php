...
function syncSingleCategory(category_id_from_k,pageindex,limitval,searchtext)
{
 ...
 // update category_entry
        echo $chk="select COUNT(*) as totalcount,MAX(priority) AS maxapriority from category_entry_priority
        where category_id='$onlycatid' and entryid='$updateentryID";
        $fetchEntryCount=  db_select($conn,$chk);
        $tcount1=$fetchEntryCount[0]['totalcount']; 
        if($tcount1==0)
        { 
          if($maxapriority>0){ $cpriority=$maxapriority+1; }
          else{ $cpriority=1;}
          
          
          echo $qry_insert1="insert into category_entry_priority(entryid,category_id,priority)
          values('$updateentryID','$onlycatid','$cpriority')";
        ...
        /*$cpriority=1;
        if($tcount1>0)
        { echo "if"; $cpriority=$maxapriority+1;  }
        else
        {
          echo $qry_insert1="insert into category_entry_priority(entryid,category_id,priority)
          values('$updateentryID','$onlycatid','$cpriority')";
