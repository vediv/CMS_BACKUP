...
switch($action)
{
   ...
   <?php  break;
    ...
    break;
        ...
        echo json_encode(array('status' =>1,'data' =>$result,'totalRecord'=>$totalRecord));
        break; 
        case "content_partner_with_addRevenue":
        $from=$_POST['fromDate']; $toDate=$_POST['toDate']; $puserid=$_POST['puserid'];
        $querySub ="SELECT name from publisher where par_id='$puserid' and acess_level='c'"; 
        $fetchTotal = db_select($conn,$querySub);
        $content_partner_name=$fetchTotal[0]['name']; 
        $qryAds="select SUM(amount) AS adsAmount from ads_revenue where (tag='ads_revenue')
                and (month  BETWEEN MONTH('$fromDate') and MONTH('$toDate')) and ( year BETWEEN YEAR('$fromDate') and YEAR('$toDate'))";
        $fetchads = db_select($conn,$qryAds);
        $adsAmount=$fetchads[0]['adsAmount'];
        $row2=array('content_partner_name' =>$content_partner_name,'TotalAdsAmount' =>$adsAmount);
        $result[]= $row2;
        echo json_encode(array('data' =>$result));
        db_close($conn);
        break;
