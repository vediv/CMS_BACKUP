  ...
  <div class="form-inline">
       <div class="form-group">
          <label for="from">Video Entry:</label>
                   <input type="text" name="VideoEntry"  size="24"  id="entryid"  placeholder="Select Video"  value="<?php echo $entry_id; ?>"  />
                   <input type="hidden" name="entryid"   size="24" id="entry_id"    />
...
$qryAds="select SUM(upd.amount) AS adsAmount FROM wallet_trans_info wti left join user_payment_details upd ON wti.orderid=upd.orderid left join user_registration ur ON wti.userid=ur.uid where order_status='Success' ";
$qryAds_1="select SUM(upd.amount) AS adsAmount FROM wallet_trans_info wti left join user_payment_details upd ON wti.orderid=upd.orderid left join user_registration ur ON wti.userid=ur.uid where order_status='Payment Initiated' ";
$qryAds_2="select SUM(upd.amount) AS adsAmount FROM wallet_trans_info wti left join user_payment_details upd ON wti.orderid=upd.orderid left join user_registration ur ON wti.userid=ur.uid where order_status!='Payment Initiated' AND order_status!='Success' ";
if($entry_id!='')
{
   $query.=" where entryid='$entry_id'";
   $qryAds.="AND wti.entryid='$entry_id'";
   $qryAds_1.="AND wti.entryid='$entry_id'";
   $qryAds_2.="AND wti.entryid='$entry_id'";
}
if($email!='')
{
   $query.=" where uemail='$email'";
   $qryAds.="AND ur.uemail='$email'";
   $qryAds_1.="AND ur.uemail='$email'";
   $qryAds_2.="AND ur.uemail='$email'";
}
// if($toDate!='' && $fromDate!=''){
//   $qryAds.= " AND (Date(purchase_date) BETWEEN  '$fromDate' AND '$toDate' )";
//   $qryAds_1.= " AND (Date(purchase_date) BETWEEN  '$fromDate' AND '$toDate' )";
// }
$query ; echo "<br/>";
...
$Success = $fetchads[0]['adsAmount']==NULL?0:$fetchads[0]['adsAmount'];
$fetchads_1 = db_select($conn,$qryAds_1);
$PaymentInitiated=$fetchads_1[0]['adsAmount']==NULL?0:$fetchads_1[0]['adsAmount'];
$fetchads_2 = db_select($conn,$qryAds_2);
$others=$fetchads_2[0]['adsAmount']==NULL?0:$fetchads_2[0]['adsAmount'];
...
<div class="row" style="border: 0px solid red; margin-top: -30px;">
    ...
    <div class="box-body" id="content_report" >
      <div class="row" style="border: 0px solid red; ">
          ...
          <a href="javascript:" class="myBtn btn" title="Export to Excel" onclick="exportData('.xls','pay_per_view_excel')"><i class="fa fa-file-excel-o bigger-110 green"></i></a>
          <a href="javascript:" class="myBtn btn " title="Export to pdf" onclick="exportData('.pdf','pay_per_view_pdf')"><i class="fa fa-file-pdf-o bigger-110 " aria-hidden="true"></i></a>
      </td>
      </tr>
      </table>
